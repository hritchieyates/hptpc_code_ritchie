...
int main(int argc, char** argv){
  ...
  TFile *input_file_anode1 = new TFile("raptorr_tpcdata_gain_R2083023_Waveforms_AllCuts_RiseTimeAnode1Above15.root", "READ");
  TFile *input_file_anode2 = new TFile("raptorr_tpcdata_gain_R2083023_Waveforms_AllCuts_.root", "READ");
  TFile *input_file_anode3 = new TFile("raptorr_tpcdata_gain_R2083023_Waveforms_AllCuts_.root", "READ");
  ...
  TCanvas *c1 = new TCanvas("c1", "c1", 4072, 7012);
  c1->Divide(1,3);
  TString pdf_name = "Tryptic_region2_"+region+".pdf";

  TCanvas *c2 = new TCanvas("c2", "c2", 4072, 7012);
  c2->Divide(1,3);
  TString pdf_name_2 = "Tryptic_R2083023_RisetimeAbove20_one_waveform_per_page.pdf";
  c2->Print(pdf_name_2+"[");
  c2->cd();

  TTree *gain_analysis = 0;
  
  input_file_anode1->GetObject("chargeGainAnalysisTree", gain_analysis);
  //the main "analysis"
  
  if(gain_analysis){
    std::cout << "gain analysis is" << std::endl;
    gain_analysis->SetBranchStatus("*", 0);
    //	gain_analysis->SetBranchStatus("PrintWaveformIDAnode1", 1);
    //	gain_analysis->SetBranchStatus("PrintEventAnode1", 1);
    //	gain_analysis->SetBranchAddress("PrintWaveformIDAnode1", &saved_waveform_IDs);
    //	gain_analysis->SetBranchAddress("PrintEventAnode1", &saved_events);

    	gain_analysis->GetEntry(0);

	//if(saved_waveform_IDs){
	// std::cout << "Number of saved waveforms is " << saved_waveform_IDs->size() << std::endl;
	// std::cout << "Now going to randomly pick values until we have 10 waveforms " << std::endl;
...
//		input_file_anode1->GetObject(graph_name_anode1, waveform_anode1);
		...
		TString graph_name_anode1 = "Anode1Waveform7_Event52";
                input_file_anode1->GetObject(graph_name_anode1, waveform_anode1);
                TString graph_name_anode2 = "Anode2Waveform7_Event52";
                input_file_anode2->GetObject(graph_name_anode2, waveform_anode2);
		TString graph_name_anode3 = "Anode3Waveform7_Event52";
                input_file_anode3->GetObject(graph_name_anode3, waveform_anode3);
		...
		if(waveform_anode1 && waveform_anode2 && waveform_anode3){
		  ...
		  c2->cd(1);
		  gPad->Clear();
		  copy_anode_1->GetXaxis()->SetTitle("Time (ms)");
		  copy_anode_1->GetYaxis()->SetTitle("Amplitude (mV)");
		  copy_anode_1->SetTitle("Anode 1 Sample waveforms for region " + region);
		  copy_anode_1->Draw();

		  c2->cd(2);
		  gPad->Clear();
		  copy_anode_2->GetXaxis()->SetTitle("Time (ms)");
		  copy_anode_2->GetYaxis()->SetTitle("Amplitude (mV)");
		  copy_anode_2->SetTitle("Anode 2 Sample waveforms for region " + region);
		  copy_anode_2->Draw();

		  c2->cd(3);
		  gPad->Clear();
		  copy_anode_3->GetXaxis()->SetTitle("Time (ms)");
		  copy_anode_3->GetYaxis()->SetTitle("Amplitude (mV)");
		  copy_anode_3->SetTitle("Anode 3 Sample waveforms for region " + region);
		  copy_anode_3->Draw();

		  anode_1->Add(waveform_anode1);
		  anode_2->Add(waveform_anode2);
		  anode_3->Add(waveform_anode3);
