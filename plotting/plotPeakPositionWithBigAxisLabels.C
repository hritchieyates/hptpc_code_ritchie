...
//paramsFile.push_back("/scratch0/hrichie/hptpc_ritchie/HPTPC/data/fitData/parameters_raptorr_tpcdata_gain_R2083022-R2083032_Am241.root.dat");
   ...
   //Fit Gas ageing plot and work out corrections from it
    ...
    //Double_t ageingUncertainty = ageingUncertainty = 27.54;                                                                                                                                    

    Double_t ageingMean = (Interp_1day_A1 + Interp_1day_B1 + Interp_1day_C1) / 3;
    Double_t ageingMinusMeanA1Square = (Interp_1day_A1 - ageingMean) * (Interp_1day_A1 - ageingMean);
    Double_t ageingMinusMeanB1Square = (Interp_1day_B1 - ageingMean) * (Interp_1day_B1 - ageingMean);
    Double_t ageingMinusMeanC1Square = (Interp_1day_C1 - ageingMean) * (Interp_1day_C1 - ageingMean);
    Double_t sumAgeingMinusMeanSquare = ageingMinusMeanA1Square + ageingMinusMeanB1Square + ageingMinusMeanC1Square;
    Double_t meanSquare = sumAgeingMinusMeanSquare / 2;
    Double_t RMS = sqrt(meanSquare);

    Double_t ageingUncertainty = RMS;   
   ...
   */
   ...
   //Turn peaks into gain
    ...
    if(Scheme == "SchemeCMod4"){
      ...
      // plotTitle << "Scheme C";
      xAxisName = "Potential Difference Between Anodes 2 and 1 (V)";
    ...
    if(Scheme == "SchemeBMod4" || Scheme == "SchemeBModSpecialTrigAll"){
      ...
      //plotTitle << "Scheme B";
      xAxisName = "Potential Difference Between Anodes 3 and 2 (V)";
   ...
   */
   ...
   peakGraph2->GetYaxis()->SetTitleSize(0.040);
   peakGraph2->GetYaxis()->SetTitleOffset(1.3);
   peakGraph2->GetXaxis()->SetTitleSize(0.040);
   peakGraph2->GetXaxis()->SetTitleOffset(0.9);
   ...
   if(gain == true){
     ...
     //peakGraph2->SetTitleSize(0.045);
     peakGraph2->GetYaxis()->SetTitle(TitleToSet.str().c_str());
     c1->Print(("/scratch0/hrichie/hptpc_ritchie/HPTPC/data/fitData/VoltageVsGasGain_BigAxisLabels_"+SchemeName+"_"+Anode+".pdf").c_str());
   }
   ...
   if(gain == false){
     ...
     //peakGraph2->SetTitleSize(0.045);
     peakGraph2->GetYaxis()->SetTitle(TitleToSet.str().c_str());
     c1->Print(("/scratch0/hrichie/hptpc_ritchie/HPTPC/data/fitData/VoltageVsPeakPosition_BigAxisLabels_"+SchemeName+"_"+Anode+".pdf").c_str());
   }
