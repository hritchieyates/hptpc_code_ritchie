...
//paramsFile.push_back("/scratch0/hrichie/hptpc_ritchie/HPTPC/data/fitData/parameters_raptorr_tpcdata_gain_R2083022-R2083032_Am241.root.dat");
   ...
   */
   ...
   if(gain == true){
     ...
     c1->Print(("/scratch0/hrichie/hptpc_ritchie/HPTPC/data/fitData/AUTO_CORRECTED_UNCERTAIN_GASGAIN_TEST_STDDEV"+SchemeName+"_"+Anode+".pdf").c_str());
   }

   if(gain == false){
   std::stringstream TitleToSet;
   TitleToSet << AnodeName << " Peak Position";
   peakGraph2->GetYaxis()->SetTitle(TitleToSet.str().c_str());
   c1->Print(("/scratch0/hrichie/hptpc_ritchie/HPTPC/data/fitData/AUTO_CORRECTED_UNCERTAIN_PEAKPOSITION_TEST_STDDEV"+SchemeName+"_"+Anode+".pdf").c_str());
   }
