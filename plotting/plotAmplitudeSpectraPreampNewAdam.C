...
#define DATA_DIR "/scratch0/hrichie/hptpc_ritchie/HPTPC/code/waveformData/testData"

// directory for the output root files
#define RESULT_DIR "/scratch0/hrichie/hptpc_ritchie/HPTPC/code/waveformData/testData"

// directory for the output root files
#define CUT_DIR "/scratch0/hrichie/hptpc_ritchie/HPTPC/code/waveformData/testData"

/* the standard single page plots */
void plotAmplitudeSpectraPreamp(std::string inputFileName = "test.root", bool cuts = false, int rebin = 2, double min = -50, double max = 250, std::string outFileExtension = ".png", bool changeRange = true){
  ...
  //Define fits
  TF1 *Sgnl = new TF1("Sgnl",Signal,3.8,3.4,3);
  Sgnl->SetParameters(3.4,70,1);
  ...
  for(int i = 1; i <= 8; i++){
     ...
     histoFitter << histotofit.str() << " >> AmplitudeAnode3a";
     ...
     TH1D* AmplitudeAnode3a = new TH1D("AmplitudeAnode3a", "AmplitudeAnode3a",101,3.3,3.5);
	...
	peakschiallFile << peakschiall[i5];
     ...
     legPeakMinusBaseline->AddEntry(AmplitudeAnode3a, "anode 3");
