...
#define RESULT_DIR "/scratch0/hrichie/hptpc_ritchie/HPTPC/code/plotting/testData"

// directory for the output root files
#define CUT_DIR "/scratch0/hrichie/hptpc_ritchie/HPTPC/code/plotting/testData"
...
void plotAmplitudeSpectraNewFe55(std::string inputFileName = "test.root",bool debug = false, bool cuts = true, int rebin = 25, double min = 0, double max = 450, std::string outFileExtension = ".png", bool changeRange = true){
  ...
  TF1 *fitFcn = new TF1("fitFcn",fitFunction,150,400,5);
  ...
  TF1 *Bkgr = new TF1("Bkgr",Background,100,1000,2);
  ...
  for(int i = 1; i <= 3; i++){
      ...
      TH1D* HPeakMinusBaselineAnode3 = new TH1D("HPeakMinusBaselineAnode3","HPeakMinusBaselineAnode3",101,-10,1010);
	...
	if (chisqndfpeak4 >= chisqndfpeak3){
      ...
      */
      ...
      Double_t chisqpeak1 = fitFcn->GetChisquare();                                                                                                                                                        
      Double_t ndfpeak1 = fitFcn->GetNDF();   
      Double_t peakerror12 = fitFcn->GetParError(3); 

      Double_t pars8[5] = {bkgr11,bkgr12,peak11,peak12,peak13};
      Double_t peaks[2] = {peak12,peakerror12}; 
      Double_t peakschi[4] = {peak12,peakerror12,chisqpeak1,ndfpeak1};
