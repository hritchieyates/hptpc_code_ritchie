...
void plotAmplitudeSpectraMany(std::string inputFileName = "test.root", std::string inputFile2Name = "anotherslightlydifferenttest.root",double norm = 1, bool cuts = true, int rebin = 16, double min = -50, double max = 3000, std::string outFileExtension = "png", bool changeRange = true){
  ...
  */
  ...
  AmplitudeAnode3a->SetTitle("Anode 1 Waveform Spectra");
  ...
  chargeGainAnalysisTreeb->Draw("AmplitudeAnode1 >> AmplitudeAnode3b", "", "goff");
  ...
  */
  ...
  ampSpectraCanvas->Print((outDir+"/"+inputFileName+"_and_"+inputFile2Name+"_HPeakMinusBaselineAnode1."+outFileExtension).c_str());
  ...
  sMinusBgCanvas->Print((outDir+"/"+inputFileName+"_minus_"+inputFile2Name+"HPeakMinusBaselineAnode1."+outFileExtension).c_str());
