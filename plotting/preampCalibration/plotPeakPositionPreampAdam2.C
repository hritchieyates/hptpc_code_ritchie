...
ooth.root.dat";
  ...
  */
   ...
   for(int runi = 0; runi < runNumber.size(); runi ++){
     stringstream filePath;
     filePath << "/scratch0/hrichie/hptpc_ritchie/HPTPC/data/preampData/2018/fitted_Spectra/AmplitudeAnode2parameters_raptorr_tpcdata_gain_R" << runNumber[runi] <<  "_expSmooth.root.dat";
     resultFile.push_back(filePath.str());
   }
   ...
   gr = new TGraphErrors(7,&(pulseSize[0]),&(Peaks1[0]),0,&(peakErrors1[0]));


   //   stringstream plotname;
   //   plotname << "Preamp Calibration Anode 2";
   gr->SetTitle("Preamp Calibration Anode 2");
   //   gr->SetMarkerColor(4);
   ...
   c1->Print("preampPeakPositionVsInputPulse.png");
   c1->Update();
}
