...
#define RESULT_DIR "/scratch0/hrichie/hptpc_ritchie/HPTPC/data/fitData/bkgFits"
...
void plotAmplitudeSpectraNewAm241(std::string inputFileName = "test.root", bool debug = true, int i = 3, Double_t iBkgr11 = 4, Double_t iBkgr12 = 0.005, Double_t iPeak11 = 80, Double_t iPeak12 = 210, Double_t iPeak13 = 80, Double_t iPeak21 = 80, Double_t iPeak22 = 210, Double_t iPeak23 = 80, int noBins = 50, bool cuts = true, std::string outFileExtension = ".png"){
  ...
  if(i == 3){
    ...
    TF1 *fitFcn = new TF1("fitFcn",fitFunction,20, 300,5);
    TF1 *Bkgr = new TF1("Bkgr",Background,20, 300,2);
    TF1 *fitFcn9 = new TF1("fitFcn9",fitFunction9,20,300,8);
  }
  else if(i == 1){
    TF1 *fitFcn = new TF1("fitFcn",fitFunction,6, 38,5);
    TF1 *Bkgr = new TF1("Bkgr",Background,6, 38,2);
    TF1 *fitFcn9 = new TF1("fitFcn9",fitFunction9,6,38,8);
  }
  ...
  Bkgr->SetParameters(iBkgr11, iBkgr12);
  ...
  //  int i = 3;
      ...
      TCanvas* ampSpectraCanvas = new TCanvas("ampSpectraCanvas", "ampSpectraCanvas",200,10,500,500);
      ampSpectraCanvas->cd();
      ...
      if(i == 3){TH1D* HPeakMinusBaselineAnode3 = new TH1D("HPeakMinusBaselineAnode3","HPeakMinusBaselineAnode3",noBins,-10,1014);}
      else if(i == 2){TH1D* HPeakMinusBaselineAnode3 = new TH1D("HPeakMinusBaselineAnode3","HPeakMinusBaselineAnode3",noBins,-10,302);}
      else if(i == 1){TH1D* HPeakMinusBaselineAnode3 = new TH1D("HPeakMinusBaselineAnode3","HPeakMinusBaselineAnode3",noBins,-2,42);}
	...
	HPeakMinusBaselineAnode3->Fit(fitFcn9, "R+");
      ...
      Double_t bkgrerror11 = fitFcn9->GetParError(0);
      Double_t bkgrerror12 = fitFcn9->GetParError(1);
	...
	if (chisqndfpeak4 >= chisqndfpeak3){
      ...
      */
      ...
      Double_t bkgrPars[4] = {bkgr11, bkgr12, bkgrerror11, bkgrerror12};

      /*
      //Save Bkg Parameters                                                                
      std::string bkgrFileName = outDir+"/bkgrparams_"+inputFileName+".dat";
      std::ofstream bkgrFile;
      bkgrFile.open(bkgrFileName.c_str());
      for(int ibkg = 0; ibkg < 4; ibkg++){
        bkgrFile << bkgrPars[ibkg];
        if(ibkg != 3) {bkgrFile << "\t";}
      }
	...
	*/
      ...
      HPeakMinusBaselineAnode3->GetYaxis()->SetTitleSize(0.045);
      HPeakMinusBaselineAnode3->GetYaxis()->SetTitleOffset(0.95);
      HPeakMinusBaselineAnode3->GetXaxis()->SetTitleSize(0.045);
      HPeakMinusBaselineAnode3->GetXaxis()->SetTitleOffset(0.85);
      gStyle->SetOptStat(0);
      HPeakMinusBaselineAnode3->Draw("");
      HPeakMinusBaselineAnode3->GetYaxis()->SetTitle("Counts");
      //TLegend* legPeakMinusBaseline = new TLegend(0.5, 0.6, 0.8, 0.8);
      //legPeakMinusBaseline->AddEntry(HPeakMinusBaselineAnode3, "anode 3");
      ampSpectraCanvas->Print((outDir+"/"+inputFileName+histotofit.str()+"_Paper"+outFileExtension).c_str());
      /*
