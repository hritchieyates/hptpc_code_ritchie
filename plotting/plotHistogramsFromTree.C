...
#define DATA_DIR "/scratch0/hrichie/hptpc_ritchie/HPTPC/code/waveformData/raptorr_waveform-Analysis/"

// directory for the output root files
#define RESULT_DIR "/scratch0/hrichie/hptpc_ritchie/HPTPC/code/waveformData/raptorr_waveform-Analysis/"

// directory for the output root files
#define CUT_DIR "$HPTPC_RESULTS_CUT/"

/* the standard single page plots */
void plotHistogramFromTree(std::string inputFileName = "test.root",double norm = 1, bool cuts = false, int rebin = 16, double min = -50, double max = 3000, std::string outFileExtension = "png", bool changeRange = true){
  ...
  */
  ...
  TH2D* eventNoVsWaveformNo = new TH2D("eventNoVsWaveformNo","eventNoVsWaveformNo",101,0,100,101,0,1000000);                 chargeGainAnalysisTreea->Draw("EventNo:WaveformTriggerT0NanoSec >> eventNoVsWaveformNo", "", "goff");  
  ...
  */
  ampSpectraCanvas->Print((outDir+"/"+inputFileName+"EventNovsWaveformNo."+outFileExtension).c_str());
  /*
