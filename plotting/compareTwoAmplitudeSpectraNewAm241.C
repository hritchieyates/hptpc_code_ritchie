...
#define CUT_DIR "/scratch0/hrichie/hptpc_ritchie/HPTPC/data/2020/03/raptorr_waveform-Analysis"

/* the standard single page plots */
void plotAmplitudeSpectraNewAm241(std::string inputFileName = "test.root", std::string secondInputFileName = "test2.root", bool debug = false, bool cuts = true, int rebin = 25, double min = 0, double max = 450, std::string outFileExtension = ".png", bool changeRange = true){
  ...
  std::cout << "got first file" << std::endl;

  TFile* secondInputData = new TFile((inDir+"/"+secondInputFileName).c_str());
  if(!secondInputData->IsOpen()){
    std::cout << secondInputFileName << " could not be opened" << std::endl;
    return;
  }
  std::cout << "got second file" << std::endl;

    inputData->cd();


  //Define fits
  TF1 *fitFcn = new TF1("fitFcn",fitFunction,45, 800,5);
  fitFcn->SetParameters(4,-0.01,140,140,50);

  TF1 *Bkgr = new TF1("Bkgr",Background,45, 800,2);
  Bkgr->SetParameters(4,0.005);

  TF1 *fitFcn9 = new TF1("fitFcn9",fitFunction9,45,800,8);
  ...
  for(int i = 3; i <= 3; i++){
      ...
      plotTitle << "Waveform Amplitude Spectrum Anode 3" << i;
      ...
      TH1D* HPeakMinusBaselineAnode3 = new TH1D("HPeakMinusBaselineAnode3","HPeakMinusBaselineAnode3",50,-10,1014);
      ...
      fitFcn->SetParameters(bkgr01,bkgr02,80,300,40);
      ...
      //2nd Fit
      fitFcn9->SetParameters(bkgr11,bkgr12,peak11,peak12,peak13,80,300,40);
	...
	HPeakMinusBaselineAnode3->Fit(fitFcn9, "R+");
      ...
      Double_t peakerror12 = fitFcn9->GetParError(3);
      //      Double_t peakerror12 = fitFcn9->GetParError(3);                                              
      //Double_t peakerror22 = fitFcn9->GetParError(3);                                              
	...
	if (chisqndfpeak4 >= chisqndfpeak3){
      ...
      */
      ...
      Double_t peakschifirst[4] = {peak12,peakerror12,chisqpeak1,ndfpeak1};
	...
	peaksFile << peaks[i3];
      ...
      //Save Peak 2 Mean + error + chisq + ndf
	...
	peakschiFile << peakschi[i4];
      ...
      //Save Peak 1 Mean + error + chisq + ndf                                                                                                                                                                  
      std::string peakschiArrayFirst = outDir+"/peakschifirst_"+inputFileName+".dat";
      std::ofstream peakschiFileFirst;
      peakschiFileFirst.open(peakschiArrayFirst.c_str());
      for(int i4 = 0; i4 < 4; i4++){
        peakschiFileFirst << peakschifirst[i4];
        if(i4 != 3) peakschiFileFirst << "\t";
      }
	...
	*/
      ...
      /*stats1 = HPeakMinusBaselineAnode3->GetListOfFunctions()->FindObject("stats")->Clone("stats1");
      stats1.SetX1NDC(0.6);
      stats1.SetX2NDC(0.9);
      stats1.SetY1NDC(0.6);
      stats1.SetY2NDC(0.9);
      */
      ...
      stringstream newhistotofitb;
      newhistotofit << "AmplitudeAnode" << i << " >> HPeakMinusBaselineAnode3b";

      secondInputData->cd();
      TTree* chargeGainAnalysisTreeb = (TTree*)secondInputData->Get("chargeGainAnalysisTree_anode3");
      TH1* HPeakMinusBaselineAnode3b = new TH1D("HPeakMinusBaselineAnode3b","HPeakMinusBaselineAnode3b",50,-10,1014);
      chargeGainAnalysisTreeb->Draw("AmplitudeAnode3 >> HPeakMinusBaselineAnode3b", "", "goff");

      HPeakMinusBaselineAnode3b->SetLineColor(2);
      //      secondInputData->cd();
      //      ampSpectraCanvas->cd();

      //Get Second Histogram from root file                                                                           
      gStyle->SetOptFit(1100);                                    
      HPeakMinusBaselineAnode3b->Draw("sames");
      //      legPeakMinusBaseline->AddEntry(HPeakMinusBaselineAnode3b, "anode 3b");

      ampSpectraCanvas->Print((outDir+"/"+inputFileName+"_comparedTo_"+secondInputFileName+histotofit.str()+outFileExtension).c_str());


      Double_t goodFit = HPeakMinusBaselineAnode3->Chisquare(fitFcn9, "R");
      Double_t badFit = HPeakMinusBaselineAnode3b->Chisquare(fitFcn9, "R");
      std::cout << "Good Fit = " << goodFit << std::endl;
      std::cout << "Bad Fit = " << badFit << std::endl;
      Double_t compareChi = HPeakMinusBaselineAnode3->Chi2Test(HPeakMinusBaselineAnode3b, "CHI2/NDF");      
      Double_t compareChi2 = HPeakMinusBaselineAnode3->Chi2Test(HPeakMinusBaselineAnode3b, "P");
      std::cout << "Chi2/ndf = " << compareChi << std::endl;
      /*
