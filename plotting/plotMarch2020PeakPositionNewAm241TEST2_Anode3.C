  ...
  */
  ...
  if(listFile.is_open()){
    ...
    while (std::getline(listFile, fileLine)){
      ...
      std::cout << "FileSuffix is " << listFileSuffix << std::endl;
   ...
   */
   ...
   c1->Print(("/scratch0/hrichie/hptpc_ritchie/HPTPC/data/fitData/BKGR12TEST_"+SchemeName+"_Anode3_"+".pdf").c_str());
   }

   if(gain == true){ 
   peakGraph2->GetYaxis()->SetTitle("Anode 3 Peak Position");
   c1->Print(("/scratch0/hrichie/hptpc_ritchie/HPTPC/data/fitData/PEAKPOSITIONTEST_"+SchemeName+"_Anode3_"+".pdf").c_str());
   }
